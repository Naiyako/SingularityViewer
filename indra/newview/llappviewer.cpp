...
bool LLAppViewer::initCache()
{
	...
	const S64 MB = 1024 * 1024;
	const S64 MIN_CACHE_SIZE = 64ll * MB; // 64 MB
	const S64 MAX_CACHE_SIZE = 102400ll * MB; // 100 GB
	const S64 MAX_VFS_SIZE = 25600ll * MB; // 25 GB

	S64 cache_size = (S64)(gSavedSettings.getU32("CacheSize")) * MB;
	cache_size = llclamp(cache_size, MIN_CACHE_SIZE, MAX_CACHE_SIZE);

	S64 texture_cache_size = ((cache_size * 3) / 4);
	S64 vfs_size = S64(cache_size) - S64(texture_cache_size);
	...
	gStaticVFS = LLVFS::createLLVFS(new_static_vfs_index_file, static_vfs_data_file, true, U64(0), false);
	if (!gStaticVFS)
	{
		return false;
	}
